 ...
 * Wallet: BLOKZ_SOLANA (Phantom path from bLOK mnemonic) — NOT CuNi SOLANA_PRIVATE_KEY
 * Meta: logo + website https://github.com/ceedot-rock/blokz
 */
import fs from "fs";
...
// Prefer game logo from blokz repo, then local, then public
...
Solana launch (pump.fun bonding curve = liquidity from day one).
Repo: https://github.com/ceedot-rock/blokz
...
async function main() {
  ...
  const sk =
    secrets.BLOKZ_SOLANA_PRIVATE_KEY ||
    process.env.BLOKZ_SOLANA_PRIVATE_KEY;
  if (!sk) throw new Error("Missing BLOKZ_SOLANA_PRIVATE_KEY (not CuNi key)");
  ...
  // Expected: 96V9SjAH…
  if (!creator.publicKey.toBase58().startsWith("96V9")) {
    console.warn("WARNING: unexpected pubkey — expected BLOKZ Phantom Solana wallet");
  }

  const price = await solPriceUsd();
  // Use most of balance after fee buffer; optional BLOKZ_BUY_SOL / BLOKZ_BUY_USD
  let buySol = process.env.BLOKZ_BUY_SOL
    ? parseFloat(process.env.BLOKZ_BUY_SOL)
    : process.env.BLOKZ_BUY_USD
      ? parseFloat(process.env.BLOKZ_BUY_USD) / price
      : Math.max(0, solBal - 0.025);

  const maxBuy = Math.max(0, solBal - 0.025);
  if (buySol > maxBuy) buySol = maxBuy;
  if (buySol < 0.01) {
    console.error("Need ≥ ~0.035 SOL (buy + fees). Have", solBal);
    process.exit(2);
  }
  ...
  console.log("Uploading logo + metadata to pump.fun IPFS…");
  const meta = await uploadMetadata(logoPath);
  console.log("metadataUri:", meta.metadataUri);
